...
#
# Result: `ohrs-ability-<version>.har` next to `ability.har`. The assemble /
# stamp / tar phase runs inside try/finally, so even a mid-flight failure
# (e.g. a tar error AFTER the manifest was stamped) restores package/ to its
# pack.bat state: a stale publish identity in package/oh-package.json5 would
# poison the dev ability.har that local file: consumers match by
# `@ohos-rs/ability`, so the restore must never be skipped.
#
# Invoke through pack-publish.bat (cmd.exe) — running .bat scripts directly
# from git bash / PowerShell can silently eat the first 2 chars of every line
# and fake success.
